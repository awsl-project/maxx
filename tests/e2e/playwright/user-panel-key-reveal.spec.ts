...
function startMockOpenAIServer(): Promise<{
  server: http.Server;
  port: number;
  requests: string[];
}> {
              ...
              choices: [
                {
                  ...
                  message: {
                    role: "assistant",
                    content: "mock key reveal proxy ok",
                  },
                  finish_reason: "stop",
                },
              ],
...
async function loginViaUI(
  ...
  await expect(page.locator("body")).toContainText(/User Console|用户控制台/, {
    timeout: 15000,
  });
}
...
async function memberAPI(
  ...
  if (!response.ok) {
    throw new Error(
      `Member API ${method} ${path} failed (${response.status}): ${text}`,
    );
  }
  return json;
}

test("user panel key reveal uses a controlled endpoint and eye-toggle UI", async ({
  page,
}, testInfo) => {
  ...
  const previousSettings = await adminAPI(
    "GET",
    "/settings",
    undefined,
    adminToken,
  ...
  try {
    ...
    await expect(page.getByText(/Generate|生成/)).toBeVisible({
      timeout: 10000,
    });
    ...
    const createdFullKey =
      createdBodyText.match(/maxx_[0-9a-f]{64}/)?.[0] ?? "";
    ...
    const memberLogin = await adminAPI("POST", "/auth/login", {
      username,
      password,
    });
    ...
    const tokenMetadata = await memberAPI(
      "GET",
      "/user-panel-token",
      memberToken,
    );
    ...
    const revealed = await memberAPI(
      "POST",
    ...
    await expect(
      page.getByText(/Full key is visible|完整密钥已显示/),
    ).toBeVisible();
    ...
    expect(proxyResponse.choices?.[0]?.message?.content).toBe(
      "mock key reveal proxy ok",
    );
    const markerRequests = mock.requests.filter((requestBody) =>
      requestBody.includes(marker),
    );
    expect(markerRequests).toHaveLength(1);

    const mockFlowScreenshot =
      "/tmp/maxx-user-panel-key-mock-proxy-response.png";
    await page.evaluate(
      ...
      {
        markerText: marker,
        responseText: proxyResponse.choices[0].message.content,
        mockCount: mock.requests.length,
      },
    );
    ...
    await expect(
      page.getByRole("tab", { name: /Requests|请求记录/ }),
    ).toHaveCount(0);
    await expect(
      page.getByRole("tab", { name: /Model Status|模型状态/ }),
    ).toHaveCount(0);
  } finally {
    await adminAPI(
      "PUT",
