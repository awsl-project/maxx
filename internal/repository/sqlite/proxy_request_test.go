...
func TestProxyRequestErrorStatsAggregatesCurrentFilter(t *testing.T) {
	...
	if providerCounts[7] != 2 || providerCounts[8] != 1 || providerCounts[9] != 0 {
	...
	if modelCounts["claude-sonnet"] != 2 || modelCounts["gpt-5"] != 1 || modelCounts["cancelled-model"] != 0 {
...
func TestProxyRequestDeleteFailedWithFilterDeletesAttemptsAndPreservesNonErrors(t *testing.T) {
	...
	for _, req := range []*domain.ProxyRequest{cancelled, completed, active, rejectedOtherProject} {
	...
	if err := db.gorm.Model(&ProxyUpstreamAttempt{}).
		Where("proxy_request_id IN ?", []uint64{cancelled.ID, completed.ID, active.ID, rejectedOtherProject.ID}).
		Count(&retainedAttempts).Error; err != nil {
		t.Fatalf("count retained attempts: %v", err)
	}
	if retainedAttempts != 4 {
		t.Fatalf("retained attempts = %d, want 4", retainedAttempts)
	}
}
...
func TestProxyRequestUserPanelModelStatusExcludesNoiseErrors(t *testing.T) {
	db, err := NewDBWithDSN("sqlite://:memory:")
	if err != nil {
		t.Fatalf("Failed to create DB: %v", err)
	}
	...
	requests := []*domain.ProxyRequest{
		{
			...
			EndTime:          now.Add(-10*time.Minute + 2*time.Second),
		...
		},
		{
			...
			EndTime:      now.Add(-8*time.Minute + time.Second),
		...
		},
		{
			TenantID:     1,
			InstanceID:   "test-instance",
			RequestID:    "noise-failure",
			SessionID:    "session-noise-failure",
			ClientType:   domain.ClientTypeClaude,
			RequestModel: "claude-sonnet-4",
			StartTime:    now.Add(-7 * time.Minute),
			EndTime:      now.Add(-7*time.Minute + 3*time.Second),
			Duration:     3 * time.Second,
			Status:       "FAILED",
			StatusCode:   500,
			Error:        "Internal server error: SSE error (code=500): Internal server error",
			APITokenID:   7,
		},
		{
			TenantID:     1,
			...
			SessionID:    "session-other-token",
			ClientType:   domain.ClientTypeClaude,
			RequestModel: "claude-sonnet-4",
			StartTime:    now.Add(-6 * time.Minute),
			EndTime:      now.Add(-6*time.Minute + time.Second),
			Duration:     time.Second,
			Status:       "FAILED",
			StatusCode:   500,
			Error:        "quota exhausted",
	...
	for _, request := range requests {
		if err := repo.Create(request); err != nil {
			t.Fatalf("Create(%s): %v", request.RequestID, err)
		}
	}

	rows, err := repo.GetUserPanelModelStatus(1, []uint64{7}, now.Add(-24*time.Hour), []string{
	...
	if err != nil {
		t.Fatalf("GetUserPanelModelStatus: %v", err)
	}
	if len(rows) != 1 {
		t.Fatalf("rows = %#v, want one model row", rows)
	}
	row := rows[0]
	if row.RequestCount != 2 || row.SuccessCount != 1 || row.FailureCount != 1 {
		t.Fatalf("counts = %+v, want request=2 success=1 failure=1", row)
	}
	if row.SuccessRate != 50 {
		t.Fatalf("success rate = %v, want 50", row.SuccessRate)
	}
	if row.TokensPerSecond != 10.5 {
		t.Fatalf("tps = %v, want 10.5", row.TokensPerSecond)
	}
}
