...
func (r *ProxyRequestRepository) toModel(p *domain.ProxyRequest) *ProxyRequest {
	return &ProxyRequest{
		...
		InstanceID: LongText(p.InstanceID),
		RequestID: LongText(p.RequestID),
		SessionID:                  p.SessionID,
		ClientType: LongText(string(p.ClientType)),
		RequestModel: LongText(p.RequestModel),
		ResponseModel: LongText(p.ResponseModel),
		StartTime:                  toTimestamp(p.StartTime),
		EndTime:                    toTimestamp(p.EndTime),
		DurationMs:                 p.Duration.Milliseconds(),
		IsStream:                   boolToInt(p.IsStream),
		Status: LongText(p.Status),
...
func (r *ProxyRequestRepository) toDomain(m *ProxyRequest) *domain.ProxyRequest {
	return &domain.ProxyRequest{
		...
		ClientType: domain.ClientType(string(m.ClientType)),
		RequestModel: string(m.RequestModel),
		ResponseModel: string(m.ResponseModel),
