...
interface ZaiProviderViewProps {
  provider: Provider;
  onDelete: () => void;
  onClose: () => void;
}

// Edit view for a z.ai (智谱 GLM) provider. z.ai natively serves both the
// Anthropic Messages endpoint (Claude) and the OpenAI Chat Completions endpoint
// (OpenAI); the plan (coding vs standard API) selects the OpenAI upstream root.
export function ZaiProviderView({ provider, onDelete, onClose }: ZaiProviderViewProps) {
  ...
  const initialClients = provider.supportedClientTypes ?? [];
  const [name, setName] = useState(provider.name);
  const [apiKey, setApiKey] = useState('');
  const [showApiKey, setShowApiKey] = useState(false);
  const [plan, setPlan] = useState<'coding' | 'api'>(
    provider.config?.zai?.plan === 'api' ? 'api' : 'coding',
  );
  // Default claude on for a legacy provider with no recorded client set, so the
  // edit form never opens in an invalid (nothing enabled) state.
